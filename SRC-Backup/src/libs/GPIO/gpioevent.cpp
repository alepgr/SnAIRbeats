...
#include "../CameraMaths/cam2opencv/libcam2opencv.h"
#include "../CameraMaths/cam2opencv/qtviewer/window.h"
...
namespace GPIOName {
    ...
    GPIOClass::GPIOClass(const char* chipName, int InterruptPin, int LEDPin,
        ...
        : chip(nullptr), SensorLine(nullptr), LEDLine(nullptr),
          ...
          running(true), Counter(0), sensor(sensor), Maths(Maths), Camera(Camera)
    {
        ...
        CameraMathsName::CameraMaths::Screenshot screenCallback;

        camera.registerCallback(&screenCallback);
